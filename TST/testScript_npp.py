...
BACKEND_COLOR = {
    ...
    "mt":      "C2",
    "npp":     "C7",   # new: NPP
    # distinct colors for Sobel CUDA variants (scatter legend)
    "cuda_v0": "C3",
    "cuda_v1": "C4",
    "cuda_v2": "C5",
    "cuda_v3": "C6",
...
# ----------------------------- WB timer parsing ------------------------------
# Base needles used for both CPU and CUDA branches
WB_TIMER_SUBSTR = {
    "sharpen":  ["sharpen kernel"],
    "sobel":    ["sobel fused kernel"],
    "gaussian": ["gaussian horiz", "gaussian vert", "gaussian kernel"],
}
# Additional needles specifically for NPP backend (case-insensitive)
WB_TIMER_SUBSTR_NPP = {
    "gaussian": ["npp gaussian", "npp filtergaussborder"],
    "sobel":    ["npp sobel", "npp magnitude", "npp gradient"],
    "sharpen":  ["npp sharpen", "npp filterborder"],
}
...
def compute_time_ms(op: str, backend: str, text: str,
    ...
    base_needles = [s.lower() for s in WB_TIMER_SUBSTR.get(op, [])]
    if backend.lower() == "npp":
        npp_needles = [s.lower() for s in WB_TIMER_SUBSTR_NPP.get(op, [])]
        needles = base_needles + npp_needles
    else:
        needles = base_needles

    total = 0.0
    matched_kernel = False

    # Sum kernel timers (may be multiple segments, e.g. gaussian horiz/vert)
    for msg, ms in timers:
        if any(n in msg for n in needles):
            total += ms
            matched_kernel = True

    # Optionally add copies and scale kernels (CUDA only, by design)
    if backend.lower() in ("cuda", "npp") and include_copies:
        for msg, ms in timers:
            if any(k in msg for k in COPY_NEEDLES):
                total += ms

    if backend.lower() in ("cuda", "npp") and include_scales:
        for msg, ms in timers:
    ...
    if debug:
        ...
        if not matched_kernel:
            print(f"[debug] no kernel match for op='{op}' backend='{backend}' using needles={needles}")
        else:
            added = []
            if backend.lower() == "cuda" and include_copies: added.append("H2D/D2H")
            if backend.lower() == "cuda" and include_scales: added.append("scale in/out")
            if added:
...
def run_one(exe: Path,
    ...
    # CUDA Sobel variant passthrough
...
def backend_sort_key(bk):
    """CPU -> MT -> NPP -> CUDA variants -> CUDA -> others."""
    if bk == "cpu":  return (0, 0)
    if bk == "mt":   return (1, 0)
    if bk == "npp":  return (2, 0)
...
def plot_scatter_time_vs_area(rows, out_path: Path):
    ...
    for r in rows:
        ...
        # split CUDA Sobel by variant for color
        ...
        xs.append(area)
        ys.append(t)
        cs.append(BACKEND_COLOR.get(color_key, "C7"))
    ...
    # Legends
    color_handles = []
    for key in sorted(color_keys_present, key=backend_sort_key):
...
def finalize_outputs(results, rows_for_csv, args, dims, plots_dir: Path):
    ...
    for op in args.ops:
        ...
        backend_keys = sorted(set(backend_keys), key=backend_sort_key)
...
def discover_dataset_inputs(dataset_dir: Path):
    ...
    for sub in dataset_dir.iterdir():
        ...
        if cand.exists():
            indices.append(idx); inputs[idx] = cand; continue
        alts = list(sub.glob("input*.ppm"))
        if alts:
            indices.append(idx); inputs[idx] = alts[0]
    indices.sort()
    return indices, inputs
...
def main():
    ap = argparse.ArgumentParser(
        description="Prepare dataset, run CUDA/CPU/MT/NPP backends, parse WB timers, plot comparisons, export CSV."
    )
    ap.add_argument("--exe", required=True, help="Path to built .exe")
    ap.add_argument("--skip-prep", action="store_true",
                    help="Skip preprocessing: reuse existing Dataset/<idx>/ subfolders.")
    ...
    ap.add_argument("--backends", nargs="+", default=["cuda", "cpu", "mt"],
                    help="Backends to run (subset of: cuda cpu mt npp)")
    ap.add_argument("--opensource-compare", action="store_true",
                    help="If set, automatically include the 'npp' backend in the run.")
    ...
    ap.add_argument("--scatter", action="store_true",
                    help="Emit a single scatter plot: time (ms, log) vs image area")
    ...
    # auto-append NPP if requested
    if args.opensource_compare and "npp" not in [b.lower() for b in args.backends]:
        args.backends.append("npp")
    ...
    # Extend BACKEND_DISPLAY with nice labels for the requested CUDA sobel variants
    ...
    rows_for_csv = []
    dims = {}      # idx -> (w,h)
    run_items = [] # (idx, input_ppm_path, image_name_for_csv)
            ...
            for op in ops_to_run:
                ...
                for backend in args.backends:
                    if op == "sobel" and backend == "cuda":
                        ...
                        # Single run for cpu/mt/cuda(non-sobel)/npp
                        try:
                            ok, t_ms, threads_used, out_text = run_one(
                                exe=exe,
                                input_ppm=input_ppm,
